...
fn main() {
    ...
    let (tx, rx): (crossbeam::Sender<&str>, crossbeam::Receiver<&str>) = channel::unbounded();
    let rx1 = rx.clone();

    let chanel_a = thread::spawn(move || {
        for msg in rx {
            println!("Chanel A: Received {}", msg);
        }
    });

    let chanel_b = thread::spawn(move || {
        for msg in rx1 {
            println!("Chanel B: Received {}", msg);
        }
    });
    ...
    let arr = ["❤️", "👨🏻‍💻", "😍", "😘"];
    for s in arr.iter() {
        println!("Main thread send {}", s);
        tx.send(s).unwrap();
    }

    drop(tx);

    chanel_a.join().unwrap();
    chanel_b.join().unwrap();
